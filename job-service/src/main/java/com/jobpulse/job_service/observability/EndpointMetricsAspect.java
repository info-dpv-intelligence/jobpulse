package com.jobpulse.job_service.observability;
...
public class EndpointMetricsAspect {
     ...
     */
    @Around("execution(* com.jobpulse.job_service.controller..*.*(..))")
    ...
    private void recordOperationSpecificMetrics(String operation, String userRole, boolean success) {
        ...
        switch (operation) {
            case "job_search":
                Counter.builder("job_searches_total")
                        .description("Total job search operations")
                        .tags(operationTags)
                        .register(meterRegistry)
                        .increment();
                break;
            ...
            case "job_creation":
                Counter.builder("job_creations_total")
                        .description("Total job creation operations")
                        .tags(operationTags)
                        .register(meterRegistry)
                        .increment();
                break;
            ...
            case "job_listing":
                Counter.builder("job_listings_total")
    ...
    private String determineOperation(String methodName, String uri) {
        ...
        if (methodName.contains("Job") && uri.contains("/jobs")) {
            if (methodName.toLowerCase().contains("create") || uri.matches(".*POST.*")) {
                return "job_creation";
            } else if (methodName.toLowerCase().contains("list") || methodName.toLowerCase().contains("get")) {
                return "job_listing";
